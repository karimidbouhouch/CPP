...
/*   Updated: 2022/09/06 20:59:37 by kid-bouh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (std::getline(std::cin, text) && !std::cin.eof())
	{
		...
		if (text == "SEARCH")
		{
			...
			if (index >= 0 && index < tmp)
			{
				...
				std::cout << "-----------------------------------------------" << std::endl;
				std::cout << index + 1 << "          |" ;
				if (phone.contact[index].getFirstname().size() > 10)
					std::cout << (phone.contact[index].getFirstname()).substr(0,10) + '.' << "|" ;
				else
				{
					std::cout << (phone.contact[index].getFirstname()).substr(0,10) ;
					j = 0;
					while (j < 11 - phone.contact[index].getFirstname().size())
					{
						std::cout << " ";
						j++;
					}
				...
				if (phone.contact[index].getLastname().size() > 10)
					std::cout << (phone.contact[index].getLastname()).substr(0,10) + '.' << "|" ;
				else
				{
					std::cout << (phone.contact[index].getLastname()).substr(0,10) ;
					j = 0;
					while (j < 11 - phone.contact[index].getLastname().size())
					{
						std::cout << " ";
						j++;
					}
